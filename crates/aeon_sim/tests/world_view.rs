...
fn opinion_pairs_are_keyed_from_then_to() {
    ...
    host.advance_days(3);
    let view = context_value(host.world_mut());
    let pairs = view["opinions"].clone().cast::<Array>();
    let by_pair = view["opinion_by_pair"].clone().cast::<Map>();
    assert!(!pairs.is_empty());
    assert_eq!(by_pair.len(), pairs.len(), "one key per ordered pair");
    for pair in pairs {
        let pair = pair.cast::<Map>();
        let key = format!(
            "{}:{}",
            pair["from"].as_int().unwrap(),
            pair["to"].as_int().unwrap()
        );
        assert_eq!(
            by_pair.get(key.as_str()).and_then(|v| v.as_int().ok()),
            pair["value"].as_int().ok(),
            "opinion_by_pair[{key}] is the pair's value"
        );
    }
}
